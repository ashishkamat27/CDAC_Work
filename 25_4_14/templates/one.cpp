...
/* The compiler can not generate any code by just looking at function
template,because it does not know which data type ,the function is
going to work on.
In the above program when compiler encounter the function call,
i.e. add(20,40) ,it knows the type to use is int,because we pass two
integers. Now it generates a specific version on the add() for 
type int,replacing every "type" with an int.This process is known
as instantiating the function template.The compiler also generates
a call to the newly instantiated function,and inserts it into the
code where add(20,40) is.
...
More Importantly,The compiler generates only one version of add()
for each data type irrespective of the number of calls that have been
made for that type.
